...
class GlobalImagePlan(BaseModel):
    ...
    images: List[ImageSpec] = Field(default_factory=list, max_length=1)
...
def _inline_schema(schema: type) -> str:
    import json
...
def structured_output(schema: type, messages: list):
    schema_str = _inline_schema(schema)
    ...
    for _ in range(3):
        ...
        try:
            ...
            instruction = SystemMessage(
                content="The JSON you returned was invalid or incomplete. Return ONLY the completed valid JSON object, nothing else."
            )
...
Decide if a single image/diagram is needed for THIS blog.

Rules:
- Max 1 image total.
- The image must materially improve understanding (diagram/flow/table-like visual).
